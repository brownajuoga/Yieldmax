...
type Repository interface {
	LoadAll() ([]Guidance, time.Time, error)
}
...
func (r *JSONRepository) LoadAll() ([]Guidance, time.Time, error) {
	...
	if !needReload {
		r.mu.RLock()
		defer r.mu.RUnlock()
		return r.entries, r.latestVersion, nil
	}

	var allEntries []Guidance
	...
	for _, file := range files {
		...
		allEntries = append(allEntries, set.Entries...)
	...
	return allEntries, maxVersion, nil
}
